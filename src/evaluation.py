...
=============================================================
 ...
 05_evaluation.py
=============================================================
 Évaluation complète des métriques ML (2022-2024).
 Produit :
 ...
 Corrections apportées :
   ✔ FP/FN analysés sur jeu de TEST uniquement (pas tout le dataset)
   ✔ Split chronologique cohérent avec entrainement_ml.py
   ✔ Kc dynamique visible dans les métriques par saison
=============================================================
"""
...
def charger() -> tuple:
    """
    Charge les données et applique le split chronologique 80/20.
    CORRECTION : évaluation sur TEST uniquement — jamais sur TRAIN.
    """
    ...
    # Régression : jours irrigués du test uniquement
...
def evaluer_classification(clf, X_all, y_all, X_te, y_te) -> tuple:
    ...
    print("  ⚠  Métriques calculées sur le jeu de TEST uniquement")
    print("═" * 62)
    ...
    # CV temporelle sur tout le dataset
    print(f"\n  Validation croisée TimeSeriesSplit (5-fold, 1096 jours) :")
    print(f"  {'─'*47}")
...
def evaluer_regression(reg, X_te_r, y_te_r) -> tuple:
    ...
    print(f"\n  Métriques TEST ({len(y_te_r)} obs irrigués) :")
    print(f"  {'─'*47}")
    ...
    interp = ("🟢 Excellent" if r2 >= 0.95 else
               "🟡 Bon"      if r2 >= 0.85 else
               "🔴 À améliorer")
    print(f"\n  Interprétation : {interp} ({r2*100:.1f}% variance expliquée)")
...
def analyse_annee_saison(clf, df: pd.DataFrame, test: pd.DataFrame):
    """
    Analyse par année et saison sur le jeu de TEST.
    CORRECTION : uniquement sur test (pas tout df).
    """
    print("\n" + "═" * 62)
    print("  ANALYSE PAR ANNÉE — jeu de TEST")
    print("═" * 62)

    print(f"\n  {'Année':<8} {'Jours':>6} {'Irrig%':>7} {'Acc%':>7} "
          f"{'Kc moy':>8} {'Pluie/an':>10}")
    print(f"  {'─'*54}")
    for yr, grp in test.groupby("annee"):
        ...
        print(f"  {yr:<8} {len(grp):>6} {irr:>6.1f}% {acc:>6.1f}% "
              f"{kc_m:>7.4f} {pluie:>9.0f}mm")

    print(f"\n  ANALYSE PAR SAISON CI (jeu de TEST) :")
    print(f"  {'─'*54}")
    print(f"  {'Saison':<24} {'Jours':>6} {'Irrig%':>7} {'Acc%':>7} {'Vol moy':>9}")
    print(f"  {'─'*54}")
    noms = {
        "seche"        : "Sèche (nov–mars)",
    ...
    for code, label in noms.items():
        ...
        print(f"  {label:<24} {len(grp):>6} {irr:>6.1f}% "
              f"{acc:>6.1f}% {vol:>8.0f}L")


# ══════════════════════════════════════════════════════════════
# 5. ERREURS CRITIQUES (sur TEST uniquement)
# ══════════════════════════════════════════════════════════════

def analyser_erreurs(clf, test: pd.DataFrame) -> tuple:
    """
    CORRECTION : FP/FN analysés sur jeu de TEST uniquement.
    L'ancienne version les cherchait sur les 1096 jours complets
    (incluant le train), ce qui ne constituait pas une évaluation honnête.
    """
    print("\n" + "═" * 62)
    print("  ANALYSE DES ERREURS CRITIQUES (jeu de TEST)")
    print("═" * 62)
    ...
    faux_neg = test[FN_mask]
    faux_pos = test[FP_mask]

    print(f"\n   FAUX NÉGATIFS — oublis d'irrigation : {len(faux_neg)} jours")
    if len(faux_neg) > 0:
    ...
    if len(faux_pos) > 0:
        ...
        print("  ✅ Aucun gaspillage d'eau détecté sur le jeu de test !")
...
def generer_graphiques(
    ...
    ax1.set_xticklabels(["Prédit NON", "Prédit OUI"], fontsize=9)
    ax1.set_yticklabels(["Réel NON", "Réel OUI"],    fontsize=9)
    labs = [["VN", "FP"], ["FN", "VP"]]
    for i in range(2):
        for j in range(2):
    ...
    ax3.set_ylabel("Précision", fontsize=10)
    ax3.set_title("Courbe Précision–Rappel", fontweight="bold", fontsize=11)
    ...
    bar_c = [C["orange"] if v > vol_mois.mean() else C["clair"]
             for v in vol_mois.values]
    ...
    bars9 = ax9.bar([str(y) for y in annees_test], accs_yr,
    ...
    ax10.set_title(
        " Tableau récapitulatif — Métriques jeu de TEST (évaluation honnête)",
        fontweight="bold", fontsize=12, pad=20,
    )

    fig.suptitle(
        "Évaluation complète — Système d'Irrigation Intelligente CI\n"
        "Tomate | 200m² | Yamoussoukro | Dataset 2022-2024 | "
...
def bilan_final(acc, prec, rec, f1, roc_auc,
    ...
    def badge(v, ok, bon):
        return ("🟢 Excellent" if v >= bon else
                "🟡 Bon"      if v >= ok  else
                "🔴 À améliorer")

    print("\n" + "═" * 62)
    print("   BILAN FINAL — MÉTRIQUES (jeu de TEST chronologique)")
    print("═" * 62)
    print(f"\n  CLASSIFICATION :")
    print(f"  {'─'*52}")
    ...
    print(f"  MAE        : {mae:>6.1f} L  "
          f"{'🟢 Excellent' if mae<80 else '🟡 Bon' if mae<120 else '🔴 À améliorer'}")
    ...
    print(f"\n  Fichiers générés : outputs/evaluation_complete.png")
    print("═" * 62)
